...
- `README.md` — build, start, shell and version-selection instructions.

## Runtime behavior
The container stays alive for interactive development and uses `/workspace` as its working directory. It will not run Docker-in-Docker and will not contain project-specific databases or services; those can be added later as separate Compose services when needed.
